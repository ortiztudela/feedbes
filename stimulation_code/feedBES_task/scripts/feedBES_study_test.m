...
for cRep=1:2
    ...
    for cPairs=1:p.learn.nTrials
        ...
        % Display
        t1=GetSecs;
        Screen('DrawTexture',w, blank, [],[centerX-700 centerY-500 centerX+700  centerY+300]);
        Screen('DrawTexture',w, scn_tex, [],[centerX-700 centerY-500 centerX+700 centerY+300]);
        Screen('DrawTexture',w, patch, [],[centerX-700 centerY-500 centerX+700 centerY+300]);
        Screen('DrawTexture', w, obj_tex(1) ,[], [centerX*.5-100 centerY+350 centerX*.5+100 centerY+450]);
        DrawFormattedText(w, '1',centerX*.5,centerY+500,white);
        Screen('DrawTexture', w, obj_tex(2) ,[], [centerX*.85-100 centerY+350 centerX*.85+100 centerY+450]);
        DrawFormattedText(w, '2',centerX*.85,centerY+500,white);
        Screen('DrawTexture', w, obj_tex(3) ,[], [centerX*1.15-100 centerY+350 centerX*1.15+100 centerY+450]);
        DrawFormattedText(w, '3',centerX*1.15,centerY+500,white);
        Screen('DrawTexture', w, obj_tex(4) ,[], [centerX*1.5-100 centerY+350 centerX*1.5+100 centerY+450]);
        DrawFormattedText(w, '4',centerX*1.5,centerY+500,white);
        [~,~,FlipTimestamp,~,~,] = Screen('Flip', w);
        WaitSecs(.2);
        remainTime=FlipTimestamp-t1;
        [tecla,~]=collectResp(p.learn.testDur,choiceKeys);
        ...
        [tecla,rt]=collectResp(p.learn.check_dur-elpasedT,checkKeys);
